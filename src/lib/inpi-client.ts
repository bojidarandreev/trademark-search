...
export const client: AxiosInstance = wrapper(axios.create({
  jar: cookieJar,
...
export async function getAccessToken(): Promise<string> {
  ...
  try {
    ...
    console.log(`DEBUG: INPI_COOKIE_SETUP_URL constant is: ${INPI_COOKIE_SETUP_URL}`);
    console.log(`Attempting initial GET to actualCookieSetupUrl: ${actualCookieSetupUrl} to obtain session cookies.`);

    try {
      await client.get(actualCookieSetupUrl, { headers: { 'User-Agent': browserUserAgent, 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7' }});
      ...
      throw new APIError(`Failed initial GET to ${actualCookieSetupUrl} to obtain session cookies. Error: ${error instanceof Error ? error.message : String(error)}`, 500, { stage: "initial-cookie-setup-get", nestedError: error });
    }
    ...
    accessToken = null; xsrfTokenValue = null; tokenExpiry = null; // Reset on error
