  ...
  if (rawData && Array.isArray(rawData.results)) {
    const mappedResults: Trademark[] = rawData.results.map((item: any) => {
      ...
      if (classNumberField) {
        ...
        if (classNumberField.value) { // Handles single value case
          classNumbers = [classNumberField.value];
        } else if (Array.isArray(classNumberField.values) && classNumberField.values.length > 0) { // Handles array of values
