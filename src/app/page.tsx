...
async function searchTrademarks(query: string): Promise<Trademark[]> {
  if (!query.trim()) {
    // Prevent fetch if query is empty or just whitespace
  ...
  if (rawData && Array.isArray(rawData.results)) {
    const mappedResults: Trademark[] = rawData.results.map((item: any) => {
      ...
      if (appNum) {
        ...
        else if (appNum.startsWith("EU") || item.ukey?.startsWith("CTMARK"))
          origine = "EU";
        else if (appNum.startsWith("WO") || item.ukey?.startsWith("TMINT"))
          origine = "WO";
      }
      ...
      let produitsServicesText =
        findFieldValue(fieldsArray, "NiceClassDetails") || "N/A";
      ...
      if (
        ...
        try {
          ...
          if (Array.isArray(niceClasses)) {
            produitsServicesText = niceClasses
              .map(
                (nc) =>
                  `Class ${nc.classNumber}: ${
                    nc.classDescription?.descriptionText || ""
                  }`
              )
              .join("; ");
          } else if (
            typeof niceClasses === "object" &&
            niceClasses.classNumber
          ) {
            produitsServicesText = `Class ${niceClasses.classNumber}: ${
              niceClasses.classDescription?.descriptionText || ""
            }`;
          } else {
      ...
      } else if (typeof niceClassField?.values !== "undefined") {
        // If it's an array of strings (less likely for complex data)
      ...
      return {
        ...
        dateDepot:
          findFieldValue(fieldsArray, "RegistrationDate") ||
          findFieldValue(fieldsArray, "PublicationDate") ||
          "N/A",
                  ...
                  {trademark.noticeUrl && (
                    <div className="md:col-span-2">
                      <Button
                        ...
                        onClick={() =>
                          window.open(trademark.noticeUrl, "_blank")
                        }
                      >
