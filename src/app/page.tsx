    ...
    const mappedResults: Trademark[] = rawData.results.map((item: any) => {
      ...
      if (niceClassField) {
        ...
        if (
          ...
          try {
            const niceClasses = JSON.parse(
              JSON.stringify(niceClassField.value)
            );
                  ...
                  .map(
                    (nc) =>
                      `Class ${nc.classNumber || "?"}: ${
                        nc.classDescription?.descriptionText || nc.text || ""
                      }`
                  )
                  .join("; ") || "N/A";
            } else if (
      ...
      if (!rawDateToFormat) {
        const pubDatesField = fieldsArray.find(
          (f) => f.name === "PublicationDate"
        );
        if (
          pubDatesField &&
          Array.isArray(pubDatesField.values) &&
          pubDatesField.values.length > 0
        ) {
