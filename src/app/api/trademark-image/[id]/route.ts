...
import { client, getAccessToken, getXsrfTokenValue, logError, APIError } from '@/lib/inpi-client';
import axios from 'axios';

const INPI_IMAGE_BASE_URL = "https://api-gateway.inpi.fr/services/apidiffusion/api/marques/image";

export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  const { id } = params;

  if (!id) {
    return NextResponse.json({ error: "Trademark ID is required for image." }, { status: 400 });
  }
  ...
  try {
    ...
    console.log(`Using X-XSRF-TOKEN for image GET: ${currentXsrf || "None (if not set)"}`);
    ...
    console.log(`Successfully fetched image for ${id} from INPI, status: ${response.status}`);
    ...
    if (error instanceof APIError) {
      return NextResponse.json({ error: error.message, code: error.statusCode, details: error.details }, { status: error.statusCode });
    }
    if (axios.isAxiosError(error)) {
        // Log the actual error response from INPI if available
        console.error("Axios error details:", error.response?.data ? Buffer.from(error.response.data).toString() : error.message);
        return NextResponse.json({ error: `Failed to fetch image: ${error.message}`, code: error.response?.status || 500 }, { status: error.response?.status || 500 });
    }
    return NextResponse.json({ error: "An unexpected error occurred while fetching the trademark image.", code: "INTERNAL_ERROR" }, { status: 500 });
  }
}
