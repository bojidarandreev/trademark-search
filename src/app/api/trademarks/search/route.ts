...
async function performSearch(bearerToken: string, searchPayload: any): Promise<any> {
    ...
    try {
        ...
        const cookiesForSearch = await client.defaults.jar!.getCookies(INPI_MARQUES_METADATA_URL);
    ...
    console.log(`Using X-XSRF-TOKEN for search: ${getXsrfTokenValue() || "None"}`);
    return client.post(
      INPI_SEARCH_URL,
      searchPayload,
      {
  ...
  try {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && (error.response?.status === 401 || error.response?.status === 403)) {
        console.log("Access token or XSRF token invalid for search, clearing auth cache and retrying search once...");
