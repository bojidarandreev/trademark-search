...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.log(`Using X-XSRF-TOKEN for notice GET: ${currentXsrf || "None (if not set)"}`);
    ...
    console.log(`Successfully fetched XML notice for ${id}, status: ${response.status}`);
    ...
    const noticeData = parsedJson.TradeMarkType || parsedJson.FrPatentDocument || parsedJson; // Adjust if root element varies
    ...
    if (error instanceof APIError) {
      return NextResponse.json({ error: error.message, code: error.statusCode, details: error.details, timestamp: new Date().toISOString() }, { status: error.statusCode });
    }
    if (axios.isAxiosError(error)) {
        return NextResponse.json({ error: `Failed to fetch notice: ${error.response?.data || error.message}`, code: error.response?.status || 500, details: error.response?.data, timestamp: new Date().toISOString() }, { status: error.response?.status || 500 });
    }
    return NextResponse.json({ error: "An unexpected error occurred while fetching the trademark notice.", code: "INTERNAL_ERROR", details: String(error), timestamp: new Date().toISOString() }, { status: 500 });
  }
}
